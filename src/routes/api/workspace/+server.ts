...
export const POST: RequestHandler = async ({ request, url }) => {
  try {
    const entityKind = kind(url.searchParams.get('kind'));
    const body = await request.json();
    if (entityKind === 'automations' && !body.nextRunAt) body.nextRunAt = nextRun(body.schedule);
    return json({ item: await createEntity(entityKind, body) }, { status: 201 });
  } catch (error) {
    const msg = error instanceof Error ? error.message : JSON.stringify(error ?? null);
    return json({ error: msg || 'Failed to create entity' }, { status: 400 });
  }
};
...
export const PATCH: RequestHandler = async ({ request, url }) => {
  try {
    ...
    const msg = error instanceof Error ? error.message : JSON.stringify(error ?? null);
    return json({ error: msg || 'Failed to update entity' }, { status: 400 });
  }
};
...
export const DELETE: RequestHandler = async ({ url }) => {
  try {
    ...
    const msg = error instanceof Error ? error.message : JSON.stringify(error ?? null);
    return json({ error: msg || 'Failed to delete entity' }, { status: 400 });
  }
};
