...
export async function listConversations() {
  return (await snapshot()).conversations.map(({ messages, ...conversation }) => ({ ...conversation, messageCount: messages.length, preview: messages.at(-1)?.content.slice(0, 120) ?? '' }));
}
...
export async function ensureConversation(id: string | undefined, messages: ChatMessage[]) {
  ...
  const conversation = {
    id: randomUUID(),
    title: messages.find((message) => message.role === 'user')?.content.slice(0, 60) || 'New conversation',
    createdAt: now,
    updatedAt: now,
    messages: [] as { id: string; role: 'user' | 'assistant' | 'system'; content: string; at: string; model?: string }[],
  };
...
export async function appendConversation(id: string, messages: { role: 'user' | 'assistant' | 'system'; content: string; model?: string }[]) {
  ...
  const now = new Date().toISOString();
  const newMessages = messages.map(message => ({ id: randomUUID(), at: now, ...message }));
  const updatedMessages = [...conversation.messages, ...newMessages];
  await updateRow('conversations', id, { messages: updatedMessages, updatedAt: now });
  await record(null, { action: 'persist-conversation', entity: 'system', entityId: id, outcome: 'success', detail: `${messages.length} message(s)` });
}

export async function deleteConversation(id: string) {
  await deleteRow('conversations', id);
}
